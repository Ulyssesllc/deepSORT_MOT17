...
def run(
    ...
    def frame_callback(vis, frame_idx):
        ...
        # np.int is deprecated; use built-in int/np.int64
        mask = results[:, 0].astype(np.int64) == frame_idx
        track_ids = results[mask, 1].astype(np.int64)
        boxes = results[mask, 2:6]
        vis.draw_groundtruth(track_ids, boxes)

        if show_false_alarms:
            groundtruth = seq_info["groundtruth"]
            mask = groundtruth[:, 0].astype(np.int64) == frame_idx
            ...
            for box in boxes:
                ...
                if iou(box, gt_boxes).max() < min_iou_overlap:
                    ...
                    vis.viewer.rectangle(*box.astype(np.int64))
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="Deep SORT result viewer for MOT17")
    parser.add_argument(
        "--sequence_dir",
    ...
    parser.add_argument(
        "--detection_file", help="Path to custom detections (optional).", default=None
    )
    parser.add_argument(
        "--update_ms",
    ...
    parser.add_argument(
        "--output_file", help="Filename of the (optional) output video.", default=None
    )
    parser.add_argument(
        "--show_false_alarms",
