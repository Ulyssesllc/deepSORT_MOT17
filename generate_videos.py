...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="Deep SORT video generator for MOT17")
    parser.add_argument(
        "--mot_dir",
    ...
    parser.add_argument(
        "--convert_h264",
        help="Convert videos to libx264 (requires FFMPEG)",
        action="store_true",
    )
    parser.add_argument(
        "--update_ms",
...
if __name__ == "__main__":
    ...
    for sequence_txt in os.listdir(args.result_dir):
        ...
        if not os.path.exists(sequence_dir):
            continue
        filename_in = os.path.join(args.output_dir, f"{sequence}.avi")
        filename_out = os.path.join(args.output_dir, f"{sequence}.mp4")
