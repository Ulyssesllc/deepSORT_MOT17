...
def gather_sequence_info(sequence_dir, detection_file):
    ...
    """
    # Validate sequence directory and image folder
    if not os.path.isdir(sequence_dir):
        raise FileNotFoundError(
            "Không tìm thấy sequence_dir: %s. Hãy đặt dữ liệu theo cấu trúc MOT17, ví dụ 'MOT17/train/MOT17-04-FRCNN'."
            % sequence_dir
        )
    image_dir = os.path.join(sequence_dir, "img1")
    if not os.path.isdir(image_dir):
        raise FileNotFoundError(
            "Không tìm thấy thư mục ảnh 'img1' trong: %s. Cấu trúc mong đợi: <sequence>/img1, <sequence>/det/det.txt, (train có thêm gt/gt.txt)."
            % sequence_dir
        )
    image_filenames = {
    ...
    if len(image_filenames) > 0:
        first_img_path = next(iter(image_filenames.values()))
        image = cv2.imread(first_img_path, cv2.IMREAD_GRAYSCALE)
        if image is None:
            raise RuntimeError(
                "Không đọc được ảnh: %s. Kiểm tra quyền truy cập/định dạng."
                % first_img_path
            )
        image_size = image.shape
    else:
        raise RuntimeError("Danh sách ảnh rỗng trong '%s'." % image_dir)
    ...
    if len(image_filenames) > 0:
        ...
        if detections is None or detections.size == 0:
            raise RuntimeError(
                "Không có ảnh và detections rỗng; không thể xác định frame range."
            )
    ...
    if os.path.exists(info_filename):
        with open(info_filename, "r") as f:
            line_splits = [line.split("=") for line in f.read().splitlines()[1:]]
            info_dict = dict(
